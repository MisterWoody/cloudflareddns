...
pub async fn check_ips_and_update_dns(
    ...
    println!("{} External IPv4 address: {}", log_prefix(), external_ipv4);
    ...
    println!("{} External IPv6 address: {}", log_prefix(), external_ipv6);
    ...
    for (host, zone) in hosts_vec.iter().zip(zones_vec.iter()) {
        ...
        println!("{} Zone ID for zone {}: {}", log_prefix(), zone, zone_id);
